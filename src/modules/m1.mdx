Margin is priced off **what a position can lose**, and the tool for seeing
...
*payoff at expiration*: x-axis is where the underlying U ends up, y-axis is your
profit or loss in dollars, for one contract (×100 shares).

### Atom 1 — Long call: pay premium, unlimited upside
...
Above K it gains $100 per $1 of stock; breakeven at K + P = $105. **Worst case is
capped at the premium.** That's why a long option's margin treatment is trivial:
pay for it in full, and nothing more is ever demanded.

### Atom 2 — Short call: collect premium, unbounded downside
...
dollar of rally — and *a stock price has no ceiling*. This is the single most
...
`is_limited_risk`, expiry ordering) exists to detect whether something is
quietly carrying one.

<RuleRef rule="short_call_uncovered" test="TestShortCallITM_p32" page="p.32" />

### Atoms 3 & 4 — Long put / short put
...
**the loss is bounded, because U stops at zero**. Worst case = (K − P) × 100.
...
When stock combines with options the payoff lines *add*, which is the whole idea
of hedged structures ([stock hedges](#m4)).
...
### Where the expiry picture stops being enough

Every chart above shows the option *at the moment it dies*. Before expiry, an
option is worth more than its intrinsic value — it carries **time value**,
...
- **The premium in the formulas is doing real work.** When the naked-short
  formula adds `P` on top of the 20% stress, it's incorporating the market's own
  ...
  without ever modeling volatility.
- **Exercise style is a risk parameter.** *American* options can be
  exercised any day; *European* only at expiry. That's why hedge rules demand
  `style: american` on the protecting leg (a hedge you can't exercise early can't
  rescue you early), and why the European box gets loan-value treatment while the American
  box doesn't ([limited risk & the catch-all](#m5)). When a `requires` guard checks `style`, this
  is what it's protecting.
- **This is the exact doorstep of risk-based margin.** Strategy-based margin
  ...
  *tomorrow* if the stock drops 12%?” Answering that requires a theoretical pricing
  ...
  that epic arrives, this section is where it plugs in.
