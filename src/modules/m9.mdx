[The previous module](#m8) covered the decisions that protect the *number*. These protect the
*proof* — determinism, audit, replay, and the system's boundaries. They matter
...
### Engine purity — the foundation everything else stands on

Three decisions from the audit epic form one idea: **the engine is a pure
function**. Same input, same bytes out, forever.
...
### Content addressing — names that can't lie
...
### Bitemporality — two clocks, because ops backdates
...
### The supersession arc — your decisions have a history, and that's the point

Trace one thread through four epics and watch locked decisions die honestly:

- **audit D7:** Mongo lives in a separate `pkg/mongo` submodule —
  keep the driver out of the minimal core, because limen might be embedded someday.
- **durable-deployment D1:** supersedes D7 — dissolve the submodule into core,
  ...
  *then*.”
- **go-sdk-facade D1–D9:** the embed use case became real (the bookkeeping
  host). Public facade types in `pkg/limen`; host owns refdata, jobs, provider
  recon; house overlay is the default mode; Mongo behind `pkg/limenmongo` (D8 —
  and note: *not* re-split into a submodule; one module, public package).
- **sdk-only-surface (#227):** the API itself deleted; recon semantics
  extracted to a normative spec the bridge implements. The entire api-first-recon ADR now
  describes deleted code — kept as history, because the *reasoning* (async jobs for
  durability, idempotency keys, trace endpoints) survives as design vocabulary even
  though the code didn't.
...
### The SDK boundary — what limen is, settled in nine decisions
...
### Trace shape for decomposition — replay the search, not just the parts
...
### How to take the next one

Distilling your own decision history into the checklist it implies — this is what every
card above has in common:

1. **Find the silent-wrong-number option first.** In almost every fork, one
   option could produce a plausible-but-wrong result with no signal. Identify it and the
   decision usually takes itself.
2. **State what each option protects, in domain terms.** “Under-margining risk
   vs recon noise,” not “fewer files touched.”
3. **Price it on a concrete position.** $5,000 vs $10,000 on the 2× ETF beat
   any abstract argument.
4. **Ask what the choice does to your measurement instruments** — recon
   buckets, trace dedup, replay. Several of your best calls (calendar ordering, error-vs-NoRule
   taxonomy, the trace wrapper) were really about keeping the oracle clean.
5. **Lock it in writing with its reasoning; supersede explicitly.** The arc
   above only worked because every reversal cited what it replaced.
