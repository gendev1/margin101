...
is where **coverage arithmetic** and the rulebook's `requires` guards earn their keep.

### Coverage arithmetic — the unglamorous load-bearing check
...
obligations are *naked* — but a formula that prices “covered call” against all
300 would silently under-margin. Every hedged rule in the book therefore carries a
`min_fields` guard (shares ≥ qty × mult) and a same-underlying guard. Shares
of the *wrong stock* cover nothing.
...
### Covered call — the canonical hedge (manual p.47)
...
**min(U, K)**: above the strike, the extra value isn't yours (you sold it),
so it can't count as collateral either. Worked p.47: 100 sh @ 92.38, short 90 call →
initial 0.50 × 92.38 × 100 = **$4,619**; maintenance
0.25 × min(92.38, 90) × 100 = **$2,250**.

### Protective put — insurance changes the floor
...
**$1,800**. Insurance lowered the requirement — and if the insurance is bad
(K far below U), the `min` hands you back to plain stock treatment.

### Collar — both hedges at once (manual p.61)
...
= **$475**. A boxed position is nearly riskless and the number says so.

### Conversions — hedged to the bone

<RuleRef rule="conversion / reverse_conversion" test="TestConversionMaintenance_p59" page="pp.59–61" />

A **conversion** is a collar with the strikes collapsed: long stock + long
put + short call *at the same strike K*. Payoff is a flat line — outcome
identical at every U. Maintenance: `10% × K × shares`, a token charge for
assignment mechanics on a position with no price risk. The
**reverse conversion** (short stock + long call + short put) is the mirror,
...
### ETF-hedged index calls — coverage across instruments

<RuleRef rule="short_index_call_long_etf" test="TestShortIndexCallLongETF_margin_p14" page="p.14" />

A short *index* call can be covered by shares of an ETF that tracks that index —
if the dollar value covers the obligation (`shares × price ≥ U × qty × mult`),
the ETF actually tracks (`tracks_index` matches), and it isn't leveraged. A 2×
ETF does *not* hedge 1× index exposure dollar-for-dollar through time — its
...
### When contracts stop being standard — the corporate-actions frontier

Everything above assumed `mult = 100` and one share class. Production breaks
both assumptions constantly, and this is the gray box on the platform map that bites
hedged rules hardest:

- **Splits.** A 2:1 split turns your 100-share contract into a 200-share one
  (or two contracts) with halved strikes. Mechanical — *if* the refdata updated. A
  hedge check against stale `mult` sees phantom over- or under-coverage.
- **Non-standard deliverables.** After a merger or special dividend, one
  contract might deliver “153 shares of NEWCO plus $4.20 cash.” Now `qty × mult`
  is not even the right *question* — coverage means matching the deliverable basket.
  The engine's clean arithmetic needs an adjusted-contract representation it doesn't have
  yet (plane 3, not started).
- **Mixed multipliers.** Mini options (`mult = 10`) exist today.
  One standard + one mini contract on the same underlying must never net one-for-one —
  which is exactly what the `same_contract_size` guard refuses, and why your
  corpus ran a whole mixed-multiplier family against the hedge rules.
