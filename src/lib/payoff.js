...
export function intrinsicCall(K, u) { return Math.max(0, u - K) }
export function intrinsicPut(K, u) { return Math.max(0, K - u) }

export function legPayoffAt(leg, u) {
  if (leg.kind === 'stock') {
...
export function positionPayoffAt(legs, u) {
...
export function shortCallReq({ U, K, P, qty = 1, mult = 100, base = 0.20, min = 0.10, lev = 1.0 }) {
...
export function shortPutReq({ U, K, P, qty = 1, mult = 100, base = 0.20, min = 0.10, lev = 1.0 }) {
...
export function mplIntrinsic(legs) {
  const strikes = legs.filter(l => l.kind !== 'stock').map(l => l.K)
...
export const fmt0 = n => fmt(n, n % 1 === 0 ? 0 : 2)
