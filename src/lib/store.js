...
export const loadDone = () => load(K.done, {})
export const saveDone = done => save(K.done, done)
...
// ---- visits / continue ----
export function recordVisit(moduleId) {
  if (moduleId) save(K.visit, { id: moduleId, at: Date.now() })
  markActiveToday()
}
export const lastVisit = () => load(K.visit, null)
...
export function dayStreak() {
  const days = new Set(load(K.days, []))
  let streak = 0
  const d = new Date()
  for (;;) {
...
// ---- drill stats ----
// shape: { byFamily: {fam: {right,total}}, byKind: {compute|verdict: {right,total}},
//          bestRun: n, today: {date, n}, sessions: n }
export const loadDrills = () => load(K.drills, { byFamily: {}, byKind: {}, bestRun: 0, today: { date: today(), n: 0 } })

export function recordDrill({ kind, family, right, run }) {
  const s = loadDrills()
  const bump = (obj, key) => {
...
export function weakestFamilies(s, min = 4) {
...
export const accuracy = v => (v && v.total ? Math.round(v.right / v.total * 100) : null)
